...
class Backward extends Component {
    ...
    backwardh(x, h, degree) {
        switch (degree) {
            case 1:
                ...
                break;
            case 2:
                y = (this.func(x) - 2*this.func(x-(1*h)) + this.func(x-(2*h))) / pow(h, 2)
                break;
            case 3:
                y = (this.func(x) - 3*this.func(x-(1*h)) + 3*this.func(x-(2*h)) - this.func(x-(3*h))) / pow(h, 3)
                break;
            default:
                y = (this.func(x) - 4*this.func(x-(1*h)) + 6*this.func(x-(2*h)) - 4*this.func(x-(3*h)) + this.func(x-(4*h))) / pow(h, 4) 
        }
    ...
    render() {
        return(
            <div>
            <h2 style={{color: "black", fontWeight: "bold"}}>Backward O(h)</h2>
                <div>
                <Row>
                        <Col span={12}>
                    <Card
                    ...
                    >
                        ...
                        <Button type="primary" size="large" id="submit_button" onClick= {()=>this.backwardh(parseFloat(this.state.x), parseFloat(this.state.h), parseInt(this.state.degree))
                        } >Submit</Button>
